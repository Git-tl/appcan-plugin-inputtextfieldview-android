...
public class NinePatchUtils {
	...
	/***
	 * 读取assets下图片,此处修改了NinePatchDrawable不能正常读取问题， e.g Drawable bg =
	 * NinePatchTool.decodeDrawableFromAsset(this, path);
	 * 
	...
	public static Drawable wrapBitmapToDrawable(Resources res, Bitmap bitmap) {
		if (bitmap == null) {
			return null;
		}
		if (bitmap.getNinePatchChunk() == null) {
			...
			Rect paddingRect = new Rect();
			readPaddingFromChunk(bitmap.getNinePatchChunk(), paddingRect);
			return new NinePatchDrawable(res, bitmap, bitmap.getNinePatchChunk(), paddingRect, null);
		}
	}
